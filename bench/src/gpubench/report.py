...
def _spend_lines(manifest_path: Path) -> list[str]:
    """One paragraph of actual spend, when `gpubench collect` recorded Job timings."""
    if not manifest_path.exists():
        return []
    manifest = json.loads(manifest_path.read_text())
    if manifest.get("gpu_hours") is None:
        return []
    price = manifest["price"]
    return [
        f"Spend — GPU-hours: {manifest['gpu_hours']:.2f} ({len(manifest.get('jobs', []))} Jobs) · "
        f"USD: {manifest['usd']:.2f} at ${price['usd_per_hour']}/h ({price['key']}, "
        f"as of {price['as_of']}). {manifest.get('spend_note', '')}",
        "",
    ]
...
def write_summary(results_dir: Path, prices_path: Path) -> Path:
    ...
    body = [
        ...
        "",
        *_spend_lines(results_dir / "manifest.json"),
        to_markdown(rows),
