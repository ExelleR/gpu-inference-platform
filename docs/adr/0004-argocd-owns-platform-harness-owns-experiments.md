...
Argo CD owns everything under `platform/`: cluster add-ons, GPU namespace/quota/priority-class scaffolding, and the two long-lived serving paths — the baseline vLLM Deployment and the KServe `InferenceService`, both in `inference` and deployed only with `make up SERVING=true`. `gpubench` owns the `bench` namespace's experiment-time objects: the `bench-results` PVC, per-run ConfigMaps, and the Jobs it creates for each run — `gpubench run` deletes the previous run's Job and ConfigMap before applying, and finished Jobs are removed by `ttlSecondsAfterFinished` (24 hours) and re-created by the next `gpubench run`; the harness never deletes them after collection, so their logs stay inspectable for the rest of the session. The one exception on the platform side is the GPU smoke-test Job, kept out of Argo CD and applied manually via `make gpu-smoke` from `platform/gpu/manual/gpu-smoke-job.yaml`, for the same immutable-Job reason. The results PVC is `ReadWriteOnce` on a zonal persistent disk (`standard-rwo`), so only one pod can mount it at a time; `gpubench collect` attaches it sequentially — first to the benchmark Job's own pod while it runs, then, after completion, to a short-lived `bench-reader` pod used only to `kubectl cp` results out.

## Consequences

The ownership boundary is scaffolding vs. runtime, not namespace vs. namespace: Argo CD still declares the `bench` namespace shell itself, and only what happens inside it at experiment time is imperative. The targets of the platform experiments (`vllm-raw-v024`, `vllm-single`, `vllm-shared`) sit on that runtime side too: they are plain Helm releases of `platform/serving/vllm-chart` that the operator installs into `bench` before an experiment and uninstalls afterwards (`docs/runbooks/bring-up.md`), outside Argo CD, so no Git change is needed to bring one up or take it down. The RWO/zonal-PD sequencing means result collection cannot overlap a running Job, capping the harness to one experiment's results at a time. A GCS bucket read and written through Workload Identity is the planned v2 for results storage, which would drop both the RWO sequencing constraint and the manual reader-pod copy step.
