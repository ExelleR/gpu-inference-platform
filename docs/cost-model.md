...
throughput (`output_throughput` from vLLM's own benchmark JSON), meaned across the three runs at
a given (variant, GPU, concurrency) point per `docs/methodology.md`. A blended variant divides by
