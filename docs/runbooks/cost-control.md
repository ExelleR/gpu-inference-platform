...
- **GPU node(s)** — only while at least one GPU pod is scheduled. The GPU node-pool resource in
  ...
  idle GPU pool costs nothing. The default `make up` leaves every GPU pool at zero nodes; a node
  appears only while a benchmark Job (or a hand-installed target release) needs it.
...
- Nothing else. `make down` destroys the VPC, NAT, cluster, node pools and the `bench-results`
  persistent disk (collect results first — `docs/runbooks/bring-up.md`); the bootstrap stage
...
sent to `alert_email` from `terraform.tfvars`. Check current spend against it on the Console
budget page — **Billing → Budgets & alerts** for the account the project is linked to; from the
CLI, `gcloud billing accounts list` shows that account's ID and name, but spend itself is only
reported in the Console (or a billing export).
...
benchmark runs. Two things keep GPU nodes alive anyway: the serving tier (`SERVING=true`), whose
pods run until `make down`, and the hand-installed target releases for experiments 06/07 until
you `helm uninstall` them. You still need to run `make down` deliberately at the end of a session,
since the system pool and NAT never scale to zero on their own.
...
is up regardless of GPU usage (see above). With `SERVING=true` the two serving nodes consume this
envelope at roughly twice the rate, before any benchmark Job runs.
